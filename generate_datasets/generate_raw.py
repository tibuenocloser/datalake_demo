...
fake = Faker('pt_PT')  # Configurando Faker para gerar dados em português
...
def gerar_dados_cadastro(n_linhas=100000):
    ...
    for _ in range(n_linhas):
        data.append({
            ...
            'nif': fake.ssn(),  # Gerando NIF
            'cp': fake.postcode(),
            'concelho': fake.concelho(),
...
def gerar_dados_pedidos(cadastros_df, n_pedidos=100000):
    ...
    for _ in range(n_pedidos):
        nif_cliente = random.choice(cadastros_df['nif'])
        ...
        data.append({
            ...
            'endereco_entrega_freguesia': fake.freguesia(),
            'endereco_entrega_concelho': fake.concelho(),
            'endereco_entrega_distrito': fake.distrito(),
            'endereco_entrega_pais': 'Portugal',
